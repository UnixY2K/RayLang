...
lang::Type LP64::defineStructType(std::string name,
...
lang::Type LP64::defineFunctionType(
...
lang::Type LP64::defineOverloadedFunctionType(lang::Type returnType) const {
	return lang::Type(
	    true,
	    // a pointer is not a scalar as it is an address memory
	    // that references an object
...
lang::Type LP64::getVoidType() const {
	return defineScalarType("void", 0, false);
}

std::optional<lang::Type>
LP64::findScalarType(const std::string_view name) const {
...
lang::Type LP64::defineScalarType(std::string name, size_t calculatedSize,
                                  bool signedType) {
	return lang::Type{
...
lang::Type LP64::definePointerType(lang::Type returnType) const {
	return lang::Type{
	    true,         // initialized type
	    true,         // it is an scalar type
	    "//ptr",      // specified name
	    POINTER_SIZE, // 64bit/8 bytes in LP64
	    false,        // by default all scalar types are const
...
std::optional<lang::Type>
LP64::getNumberLiteralType(const std::string_view lexeme) const {
